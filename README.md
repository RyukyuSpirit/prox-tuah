# prox-tuah
## Summary
prox-tuah (Proxmox text-based user and administration handler) is a remote TUI for PVE. It can be used to walk and call the PVE API, manage a PVE, and "broker" connections to VMs. The interface handles similar to network device CLIs with tab completion and context-specific help menus, as well as output modifiers to adjust return content and formatting.
